...
import { setEntities, setFlagger, SetFlaggerPayload, setPager } from "@/store";
import { ApiResponse, PaginatedResponse } from "@matcha/shared";
...
export abstract class BaseService {
  ...
  protected handlePaginatedResponse<T>(
    ...
    const entityKeys = data.map((entity) => String((entity as any).id));
    this.dispatch(
      setPager({
        pagerKey,
        meta,
